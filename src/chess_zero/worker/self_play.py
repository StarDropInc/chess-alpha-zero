...
class SelfPlayWorker:
    def __init__(self, config: Config):
        ...
        self.m = Manager()
        self.current_pipes = self.m.list([self.model.get_pipes(self.config.play.search_threads) for _ in range(self.config.play.max_processes)])
    ...
    def start(self):
        ...
        while True:
            with ProcessPoolExecutor(max_workers=self.config.play.max_processes) as executor:
                start_time = time()
                for future in as_completed([executor.submit(self_play_buffer, self.config, self.current_pipes) for _ in range(self.config.play_data.nb_game_in_file)]):
                    game_idx += 1
